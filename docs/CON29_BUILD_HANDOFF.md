...
**Also verify before relying on it further (found while building DEF-03, 2026-08-06):** `property_resolver.py`'s Borough resolution reads `organisation-entity` off `local-authority-district` entities. That field is confirmed present on planning.data.gov.uk's **brownfield-land** dataset (Bristol = 66, Hackney = 163, both seen live in Section 3.5) but has never been confirmed on a real **local-authority-district** response specifically. If it's absent there, `_lookup_local_authority` silently returns `None` for every property, `borough` is always `None`, `PropertyOutOfScopeError` never fires for anything, and `gis_agent`/`planning_agent`'s borough-dispatch logic has nothing to key off — a silent degradation to "nothing works, nothing errors", the worst failure shape, since every existing test would still pass. One live call settles it:

```
curl -s "https://www.planning.data.gov.uk/entity.json?dataset=local-authority-district&latitude=51.452073&longitude=-2.604062"
```

Not made yet — do it before or at the start of WP-03, not left implicit.
...
**Excluded UPRNs, found while building the resolver's test fixtures (2026-08-06):**

| UPRN | Address | Reason for exclusion |
|---|---|---|
| 67678 | 122 Whiteladies Road, Bristol BS8 2RP | Commercial (`CLASSIFICATION_CODE_DESCRIPTION: Shop / Showroom`), not suitable as a locked residential test property — the fixture note itself flags this |
| 10008231087 | 41 Amhurst Road, London E8 1LL | Returned by OS Places for a search on "14 Amhurst Road" — number 14 doesn't exist on this street. Never a genuine test address; it's the roadmap's own fictional example address, kept only as a regression fixture for match-confidence surfacing |

Both UPRNs are used in `tests/fixtures/bristol/` and `tests/fixtures/hackney/` as resolver test doubles — legitimate there — but must not be reused as locked WP-11 test properties. A constraint that only lives in a fixture file's own note won't be found when the test property set is actually being chosen.

**House-number-level match verification is mandatory for every candidate, not just these two exclusions.** The Hackney case generalises beyond one UPRN: a search for "14 Amhurst Road" returned number 41 on the same street and postcode, labelled `MATCH_DESCRIPTION: "GOOD"` at 0.8 — `match_description` alone does not guarantee house-number identity. For every WP-11 candidate, record `match_score`, `match_description`, and the matched address alongside the address that was actually searched, and reject any candidate where the matched house number differs from the one searched. The table above stops the two known-bad UPRNs; this line stops the next one.
...
| 2026-08-06 | WP-01 | `src/models.py`'s first draft docstring claimed AwareDatetime under pydantic strict mode accepts ISO-8601 strings (including the roadmap's own `...Z` manifest format) as "the one documented practical exception to strict mode". Tested directly against pydantic==2.13.4 before relying on it: false. The Python constructor rejects every string form; only a real aware `datetime` object validates. | Corrected the docstring to state the tested behaviour instead of the assumed one. Separately confirmed `model_validate_json` (a different code path) DOES accept offset and `...Z` ISO-8601 strings under strict mode, so a manifest written with `model_dump_json` still reads back cleanly — that asymmetry is now documented in `src/models.py` and covered by `tests/test_models.py`. Raises an open WP-02 question: whether adapters should be moved to emit aware `datetime` objects directly now that a string can never reach `CON29Field`'s constructor, rather than continuing to emit strings for a future mapper to parse — not decided here. |
